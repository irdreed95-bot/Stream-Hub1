      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          {groups.length > 1 && !error && !isLoading && (
            <select 
              className="w-full sm:w-48 h-10 rounded-md bg-card border border-border text-sm px-3 focus:outline-none focus:ring-2 focus:ring-primary text-foreground"
              value={selectedGroup}
              onChange={e => setSelectedGroup(e.target.value)}
              data-testid="select-group-live"
            >
              {groups.map(g => (
