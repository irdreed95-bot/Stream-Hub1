...
function MovieCarousel({ title, data, isLoading }: { title: string; data: any[]; isLoading: boolean }) {
  ...
  return (
    <section className="py-4">
      <h2 className={cn(
        "text-xs sm:text-sm font-bold uppercase tracking-wider text-primary mb-3 mx-4 md:mx-8",
        isRtl ? "border-r-2 border-primary pr-3" : "border-l-2 border-primary pl-3"
      )}>
        {title}
      </h2>
      <div className="flex overflow-x-auto gap-3 px-4 md:px-8 pb-3 snap-x snap-mandatory scrollbar-hide">
        {isLoading
          ? Array.from({ length: 7 }).map((_, i) => (
              <div key={i} className="min-w-[120px] sm:min-w-[150px] md:min-w-[170px] snap-start shrink-0">
                <Skeleton className="w-full aspect-[2/3] rounded-xl" />
              </div>
            ))
          : data.map(item => (
              <div key={item.id} className="min-w-[120px] sm:min-w-[150px] md:min-w-[170px] snap-start shrink-0">
                <MovieCard
                  id={item.id}
...
export default function Home() {
  ...
  return (
    <div className="w-full min-h-screen bg-background pb-20">
      ...
      <div className="relative w-full h-[45vh] sm:h-[55vh] md:h-[72vh] bg-black overflow-hidden">
        {trendingLoading ? (
          ...
          <>
            ...
            <div className={cn(
              "absolute bottom-0 p-6 md:p-12 w-full md:w-2/3 space-y-4",
              isRtl ? "right-0 text-right" : "left-0 text-left"
            )}>
              <h1 className="text-2xl sm:text-3xl md:text-5xl font-black text-white tracking-tight drop-shadow-lg leading-tight">
                {hero.title || hero.name}
              </h1>
              <p className="text-muted-foreground line-clamp-2 text-xs sm:text-sm max-w-xl">
                {hero.overview}
              </p>
              <div className="flex items-center gap-3 pt-2">
                <Link href={`/${hero.media_type || "movie"}/${hero.id}`}>
                  <Button size="lg" className="gap-2 text-xs sm:text-sm font-bold shadow-lg shadow-primary/30">
                    <Play className="w-4 h-4 fill-current" />
                    {t("playNow")}
                  </Button>
                </Link>
                <Link href={`/${hero.media_type || "movie"}/${hero.id}`}>
                  <Button size="lg" variant="secondary" className="gap-2 text-xs sm:text-sm bg-white/15 hover:bg-white/25 text-white border-0 backdrop-blur-sm">
                ...
                {heroes.map((_, i) => (
                  <button key={i} onClick={() => setHeroIndex(i)}
                    className={cn("h-1 rounded-full transition-all duration-300",
                      i === heroIndex ? "bg-primary w-6" : "bg-white/30 w-1.5 hover:bg-white/60"
                    )} />
                ))}
              </div>
            </div>
      ...
      {/* ── Content Rows ── */}
      <div className="mt-[-2rem] sm:mt-[-4rem] relative z-10">
